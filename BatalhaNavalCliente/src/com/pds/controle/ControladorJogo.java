...
public class ControladorJogo implements Initializable, ObservadorJogo {
	...
	public void exibeResultadoJogada(String imgPath) {
		Platform.runLater(() -> {
			gridTabuleiroInimigo.add(new ImageView(imgPath), ultimaJogada.getPosX() + 1,
					ultimaJogada.getPosY() + 1);
		});
	}
	...
	public void novaJogada(Jogada jogada) {
		...
		Celula celulaAtingida = tabuleiroMeu.getCelulas().get(jogada.getPosX())
				.get(jogada.getPosY());
				...
				if (celula.equals(celulaAtingida)) {
					...
					if (embarcacao.isDestruida()) {
						...
						for (Celula c : embarcacao.getCelulas()) {
							...
							Mensagem msg = new Mensagem.MontadorMensagem(TAG.DESTROYED).jogada(j)
									.jogador(apelidoJogador).nomePartida(nomePartida)
									.imgPath(c.getImgPath()).build();
		...
		if (tabuleiroMeu.todasEmbarcacoesForamDestruidas()) {
			Mensagem msgPerdi = new Mensagem.MontadorMensagem(TAG.LOSTGAME).jogador(apelidoJogador)
					.nomePartida(nomePartida).build();
	...
	private void atualizarCelula(GridPane grid, Jogada jogada, Tabuleiro tabuleiro) {
		...
		Platform.runLater(() -> {
			ImageView node = new ImageView(tabuleiroMeu.getCelulas().get(jogada.getPosX())
					.get(jogada.getPosY()).getImgPath());
			grid.add(node, jogada.getPosX() + 1, jogada.getPosY() + 1);
		});
	}

	public void setTabuleiroMeu(Tabuleiro tabuleiro) {
		tabuleiroMeu = tabuleiro;
	}
}
